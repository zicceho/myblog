...
const BLOG = {
  ...
  AUTHOR: process.env.NEXT_PUBLIC_AUTHOR || '念安酒馆', // 您的昵称 
  BIO: process.env.NEXT_PUBLIC_BIO || '听有你的故事，等有故事的你', // 作者简介
  LINK: process.env.NEXT_PUBLIC_LINK || 'https://www.885111.xyz', // 网站地址
  KEYWORDS: process.env.NEXT_PUBLIC_KEYWORD || 'Notion, 博客, 播客', // 网站关键词 英文逗号隔开
...
// END ************网站字体*****************
  ...
  // 所有支持的分享服务：link(复制链接),wechat(微信),qq,weibo(微博),email(邮件),facebook,twitter,telegram

  // 文章URL前缀
  POST_URL_PREFIX: process.env.NEXT_PUBLIC_POST_URL_PREFIX ?? 'ep',
...
// 定义获取播客信息的函数
function getPodcastInfo(rssUrl) {
  return fetch(rssUrl)
  ...
  .then(data => {
      ...
      return Array.from(items).map(item => ({
        ...
        artist: item.getElementsByTagName('author')[0].textContent,
        url: item.getElementsByTagName('enclosure')[0].getAttribute('url'),
        cover: item.getElementsByTagName('image')[0]?.getAttribute('href') || ''
      }));
   });
}

// 获取播客信息并更新音频列表
getPodcastInfo('https://www.ximalaya.com/album/15025296.xml')
.then(podcastInfo => {
  MUSIC_PLAYER_AUDIO_LIST = podcastInfo;
})
.catch(error => {
  console.error('获取播客信息时出错:', error);
});

MUSIC_PLAYER_AUDIO_LIST: [], // 初始化为空，等待播客信息获取后更新
...
player.on('stop', () => {
  ...
  COMMENT_VALINE_PLACEHOLDER:
    process.env.NEXT_PUBLIC_VALINE_PLACEHOLDER || '来聊聊吧~', // 可以搭配后台管理评论 https://github.com/DesertsP/Valine-Admin  便于查看评论，以及邮件通知，垃圾评论过滤等功能
  ...
  TITLE: process.env.NEXT_PUBLIC_TITLE || '念安酒馆', // 站点标题 ，被notion中的页面标题覆盖；此处请勿留空白，否则服务器无法编译
  HOME_BANNER_IMAGE:
    process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE || '/bg_image.jpg', // 首页背景大图, 会被notion中的封面图覆盖，若无封面图则会使用代码中的 /public/bg_image.jpg 文件
  DESCRIPTION:
    process.env.NEXT_PUBLIC_DESCRIPTION || '听有你的故事，等有故事的你', // 站点描述，被notion中的页面描述覆盖
