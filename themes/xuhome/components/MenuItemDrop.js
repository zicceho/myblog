...
export default function MenuItemDrop({ link }) {
  ...
  return (
    <div className='relative'
      ...
      {hasSub && (
        <div className={`absolute top-full left-0 mt-1 min-w-[160px] z-50 transition-all duration-150 ${show ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-1'}`}>
          <div className='border-2 border-[#0284c7] rounded-sm shadow-[4px_4px_0px_0px_#0284c7] bg-[#ffffff] dark:bg-slate-800 p-1.5 flex flex-col gap-1'>
            {link.subMenus.map((sub, i) => (
              <SmartLink key={sub.id || i} href={sub.href || sub.url} className='no-underline'>
