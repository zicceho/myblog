...
function MenuItem({ link, onClose }) {
  ...
  if (!hasSub) {
    return (
      <SmartLink href={link.href || link.url} className='no-underline' onClick={onClose}>
  ...
  return (
    <div>
      <button
        ...
        {link.name || link.title}
        <span className={`inline-block ml-2 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
          {'\u25BC'}
        </span>
      </button>

      <div className={`mt-1.5 ml-4 flex flex-col gap-1.5 transition-all duration-200 ${open ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        {link.subMenus.map((sub, i) => (
          <SmartLink key={sub.id || i} href={sub.href || sub.url} className='no-underline' onClick={onClose}>
...
export default function MobileMenu({ links, onClose }) {
  ...
  return (
    <div className={`absolute top-full left-0 right-0 z-50 border-t-2 border-[#0284c7] bg-white dark:bg-slate-800 shadow-[0px_4px_0px_0px_#0284c7] transition-all duration-200 origin-top ${show ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'}`}>
